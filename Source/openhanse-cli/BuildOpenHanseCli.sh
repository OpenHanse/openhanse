...
build_linux_target() {
  ...
  container run \
    ...
    --cpus "$CONTAINER_CPUS" \
    --env "CARGO_TARGET_DIR=/workspace/openhanse/Source/openhanse-cli/target/container-$label" \
    --env "CARGO_BUILD_JOBS=$CONTAINER_CARGO_JOBS" \
    --volume "$REPO_ROOT:/workspace/openhanse" \
    --workdir /workspace/openhanse/Source/openhanse-cli \
    "$image_tag" \
